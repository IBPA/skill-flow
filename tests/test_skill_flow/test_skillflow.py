...
class TestSkillFlow:
    ...
    def test_with_both_rerankers_calls_three_stages(self):
        ...
        reranker2.rerank.return_value = final_results

        retriever = SkillFlow(searcher, reranker, reranker2)
        results = retriever.search("query", top_k=2)

        assert results == final_results
        searcher.search.assert_called_once_with("query")
        reranker.rerank.assert_called_once_with("query", candidates)
        reranker2.rerank.assert_called_once_with("query", stage2_results, top_k=2)

    def test_with_reranker2_but_no_reranker_skips_both(self):
        searcher = MagicMock()
        expected = _make_results(3)
        searcher.search.return_value = expected
        ...
        retriever = SkillFlow(searcher, reranker=None, reranker2=reranker2)
        results = retriever.search("query", top_k=3)

        assert results == expected
        searcher.search.assert_called_once_with("query", top_k=3)
        reranker2.rerank.assert_not_called()
    ...
    def test_with_full_pipeline_calls_four_stages(self):
        ...
        reranker2.rerank.return_value = stage3_results
        ...
        searcher.search.assert_called_once_with("query")
        reranker.rerank.assert_called_once_with("query", candidates)
        reranker2.rerank.assert_called_once_with("query", stage2_results, top_k=3)
