"""Shared skill resolution and injection logic for evaluation agents."""
...
class SkillInjectionMixin(BaseAgent):
    ...
    def _resolve_from_selector_cache(self, task_name: str) -> list[Path]:
        ...
        matched_task_id = find_matching_task_id(list(cache), task_name)
        keys = cache.get(matched_task_id, []) if matched_task_id else []
        if not keys:
