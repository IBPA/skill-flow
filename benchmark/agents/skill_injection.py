"""Shared skill-injection mixin for evaluation agents.

Resolves skills from one of three mutually exclusive sources and injects the
matching SKILL.md folders into the evaluation container during ``setup``:

- ``skills_dir``: scan a local directory tree for SKILL.md files (golden/oracle
  and Vercel injection)
...
The mixin is CLI-agnostic: it cooperates with any Harbor ``BaseAgent`` subclass
(e.g. Codex or Gemini CLI) via ``super()`` so the same injection behaviour is
shared across agent backends.
"""
