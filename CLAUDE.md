...
- **`paper/`** — Manuscript hub with Overleaf sync. Shared assets at the root (`tables/`, `figures/`, `references.bib`) generated by `analysis/`; the arXiv manuscript is `paper/arxiv/main.tex`, which pulls them via `\input{../tables/...}` / `\includegraphics{../figures/...}` / `\bibliography{../references}`.
...
- **Overleaf Sync**: `paper/scripts/push-overleaf.sh` does a one-way push of the paper hub (the `arxiv/` manuscript + shared `tables/`/`figures/`/`references.bib`) to Overleaf; the Overleaf main document is `arxiv/main.tex`. `--reset` clones, replaces contents, and force-pushes on diverged histories. Requires `OVERLEAF_API_KEY` and `OVERLEAF_REPO_URL` in `.env`.
